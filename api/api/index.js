  ...
  rs(url, async (err, resp, html) => {
    if (!err) {
      try {
        ...
        var linkx =
          "https://animexninja-ap.dhvitop1.repl.co/watch/" + id + "/" + episode;

        var fid = $("div.anime_muti_link ul li.xstreamcdn a").attr(
          "data-video"
        );
        ...
        return res.status(404).json({ links: [], totalepisode: totalepisode });
      }
    }
  });
});
...
async function generateEncryptAjaxParameters($, id) {
  ...
  const cryptVal = $("script[data-name='episode']").data().value,
  ...
  return (
    "id=" +
    encryptedVideoId +
    decryptedStr.substring(decryptedStr.indexOf("&")) +
    "&alias=" +
    videoId
  );
}
...
app.get("/watch/:id/:episode", async (req, res) => {
  try {
    ...
    prev = prev.children("a").text()
      ? `https://animexninja-api.dhvitop1.repl.co/watch/${id}/${
          Number(episode) - 1
        }`
      : null;
    next = next.children("a").text()
      ? `https://animexninja-api.dhvitop1.repl.co/watch/${id}/${
          Number(episode) + 1
        }`
      : null;
...
const scrapeMP4 = async (serverUrl) => {
  ...
  let sr2 =
    sources[1] && sources[1].file.includes("m3u8") ? sources[1].file : null;
  let sourcer;
  if (sr2) {
    sourcer = sr2;
  } else {
...
app.listen(config.port, () => {
  console.log(
    `Api running on port ${config.port}. Add http://localhost:${config.port} to the website config.`
  );
});
